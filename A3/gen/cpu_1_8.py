...
for x in xrange(8):
	for y in xrange(8):
		for M in [20, 40, 60, 80, 100, 120, 140, 160]:
			f = open("run_nbody_" + str(int(cnt /150)) + "_" + str(x+1) + "_" + str(y+1) + "_" + str(M) + ".sh", "w")
			cnt += 1
			f.write(header)
			for i in xrange(5):
				f.write("mpirun -np " + str((x+1) * (y+1)) + " ./nbody-bang -t 75 -n 150000 -M " + str(M) + " -s 1382741450 -px " + str(x+1) + " -py " + str(y+1) + "\n")
			f.write(post)
			f.close()
