...
#if VALIDATION==1
		double* D = (double*) malloc(3*nmax*nmax*sizeof(double));
#endif
...
//				square_dgemm(n, A, B, D);
        ...
        for (int i = 0; i < n; i++)
          for (int j = 0; j < n; j++)
            if ((D[i*n+j] - C[i*n+j]) * (D[i*n+j] - C[i*n+j]) > 0.000001) {
							printf("MATRIX A: \n");
							printMatrix(A, n);
							printf("\n\nMATRIX B:\n");
							printMatrix(B, n);
							...
							printMatrix(D, n);
							printf("\n\n");
              printf("ERROR in algorithm for n=%d, nmax=%d: Expected %f but saw %f at (%d, %d)\n", n, nmax, D[i*n+j], C[i*n+j], i, j);
							exit(1);
						}
      }
#endif
